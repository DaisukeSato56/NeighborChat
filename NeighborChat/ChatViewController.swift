...
//    アバターを返す
    override func collectionView(_ collectionView: JSQMessagesCollectionView!, avatarImageDataForItemAt indexPath: IndexPath!) -> JSQMessageAvatarImageDataSource! {
        let message = self.messages?[indexPath.row]
        if message?.senderId == senderId {
            return self.outgoingAvatar as! JSQMessageAvatarImageDataSource!
        } else {
            return self.incomingAvatar as! JSQMessageAvatarImageDataSource!
        }
    }
...
//        吹き出しの設定
        ...
        self.outgoingBubble = bubbleFactory?.incomingMessagesBubbleImage(with: UIColor.jsq_messageBubbleBlue())
...
//    メッセージをfirebaseから取ってくる
    func getInfo() {
        ...
        let firebase = Database.database().reference(fromURL: "https://neighborchat-fa60d.firebaseio.com/").child("message").child(address).child(roomName)
